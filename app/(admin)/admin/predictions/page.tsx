...
import { Play, Clock, CheckCircle2, AlertCircle, Loader2, XCircle } from "lucide-react"
...
function statusBadge(status: string) {
  if (status === "COMPLETED") return <Badge className="bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400">Completed</Badge>
  if (status === "RUNNING")   return <Badge className="bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400">Running</Badge>
  if (status === "FAILED")    return <Badge className="bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400">Failed</Badge>
  return <Badge className="bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400">Pending</Badge>
}
...
export default function PredictionsPage() {
  ...
  function fetchRuns() {
    setLoading(true)
    api.get<PredictionRun[]>("/predictions")
  ...
  async function handleRun() {
    if (!mlHealthy) {
      toast.error("ML service is not reachable or models are not trained. Start the FastAPI service first.")
      return
    }
    setRunning(true)
    try {
      ...
      const res = await api.post<{ runId: string; totalStudents: number; flaggedStudents: number }>(
        "/predictions/run", body
      )
      toast.success(`Prediction complete — ${res.data.flaggedStudents} student(s) flagged out of ${res.data.totalStudents}`)
      fetchRuns()
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Predictions</h1>
        <p className="text-sm text-slate-500 dark:text-slate-400">
          Run the ML model to predict student performance and flag at-risk students
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <Card className="lg:col-span-1 dark:border-slate-800 dark:bg-slate-900">
          <CardHeader>
            <CardTitle className="text-base dark:text-white">Run New Prediction</CardTitle>
        ...
        <div className="space-y-4 lg:col-span-2">
          {latestCompleted ? (
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-emerald-700 dark:text-emerald-400">
                      {new Date(latestCompleted.createdAt).toLocaleString("en-NG")} ·{" "}
                      {ALGO_LABELS[latestCompleted.algorithm]} ·{" "}
                      {formatDuration(latestCompleted.durationSeconds)}
                    </p>
                    <div className="mt-3 grid grid-cols-3 gap-3">
                      {[
                        { label: "Students Analysed", value: latestCompleted.totalStudents.toLocaleString() },
                        { label: "At-Risk Flagged",   value: latestCompleted.flaggedStudents.toLocaleString() },
                        { label: "F1 Score",          value: latestMetrics ? (latestMetrics.f1Score * 100).toFixed(1) + "%" : "—" },
                      ].map(({ label, value }) => (
                        <div key={label} className="rounded-lg bg-white/70 p-2.5 dark:bg-emerald-900/20">
                          <p className="text-[10px] text-emerald-600 dark:text-emerald-400">{label}</p>
            ...
            <Card className="border-slate-200 dark:border-slate-800 dark:bg-slate-900">
              <CardContent className="flex items-center gap-3 pt-4">
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400">
                  No predictions run yet. Configure and click "Run Prediction" to get started.
                </p>
              </CardContent>
            </Card>
          )}
          ...
          {latestCompleted && (
            <div className="grid grid-cols-3 gap-3">
              {[
                { label: "Flagged (High+Medium)", value: latestCompleted.flaggedStudents, color: "text-red-600", bg: "bg-red-50 dark:bg-red-950/30" },
                { label: "Total Students",        value: latestCompleted.totalStudents,   color: "text-slate-700 dark:text-slate-200", bg: "bg-slate-50 dark:bg-slate-800/40" },
                { label: "Flagged Rate",          value: `${((latestCompleted.flaggedStudents / latestCompleted.totalStudents) * 100).toFixed(1)}%`, color: "text-amber-600", bg: "bg-amber-50 dark:bg-amber-950/30" },
      ...
      <Card className="dark:border-slate-800 dark:bg-slate-900">
        <CardHeader>
          <CardTitle className="text-base dark:text-white">Run History</CardTitle>
          <CardDescription className="dark:text-slate-400">All previous prediction runs</CardDescription>
        </CardHeader>
            ...
            <Table>
              <TableHeader>
                <TableRow className="dark:border-slate-800">
                  <TableHead className="pl-6 dark:text-slate-400">Cohort</TableHead>
                  <TableHead className="dark:text-slate-400">Date</TableHead>
                  <TableHead className="dark:text-slate-400">Algorithm</TableHead>
                  <TableHead className="dark:text-slate-400">Students</TableHead>
                  <TableHead className="dark:text-slate-400">Flagged</TableHead>
                  <TableHead className="dark:text-slate-400">F1 Score</TableHead>
                  <TableHead className="dark:text-slate-400">Duration</TableHead>
                  <TableHead className="pr-6 dark:text-slate-400">Status</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {runs.map((r) => {
                  ...
                  return (
                    <TableRow key={r.id} className="dark:border-slate-800">
                      <TableCell className="pl-6 text-sm dark:text-slate-300 max-w-[220px] truncate">{r.cohortLabel}</TableCell>
                      ...
                      <TableCell className="text-sm text-slate-500 dark:text-slate-400">{ALGO_LABELS[r.algorithm] ?? r.algorithm}</TableCell>
                      <TableCell className="text-sm dark:text-slate-300">{r.totalStudents.toLocaleString()}</TableCell>
                      <TableCell>
                        <span className="text-sm font-semibold text-red-600">{r.flaggedStudents}</span>
                      </TableCell>
